...
## �️ Векторные иконки (SVG Path)
 
 Вы можете использовать векторные иконки вместо PNG/Emoji для максимального качества.
 Просто добавьте соответствующие поля с SVG Path Data в `variables`.
 
 | Переменная | Описание |
 |------------|----------|
 | `pathHome` | Иконка "Главная" |
 | `pathVersions` | Иконка "Версии" |
 | `pathProfiles` | Иконка "Профили" |
 | `pathServers` | Иконка "Серверы" |
 | `pathThemes` | Иконка "Темы" |
 | `pathSettings` | Иконка "Настройки" |
 ...
 **Пример:**
 ...
 ## 🛠️ Кастомные стили (XAML)
 
 Вы можете полностью переопределить дизайн любого компонента, загрузив свои `Styles`.
 
 1. Создайте файл `styles.axaml` в папке темы.
 2. Лаунчер загрузит его автоматически.
 3. Если нужно несколько файлов, укажите их в `resources.styles`:
